...
template <typename Type, int diameter>
__global__ void median_general_kernel2d(const Type *in, Type *out, float dif,
                                      int M, int N) {
  constexpr int radius = diameter / 2;
  constexpr int d3 = diameter * diameter;
  constexpr int midpoint = d3 / 2;
  ...
  /* perform median filtration */
  long long index = static_cast<long long>(i) + N * static_cast<long long>(j);
  if (dif == 0.0f)
    out[index] = ValVec[midpoint];
  else {
    /* perform dezingering */
