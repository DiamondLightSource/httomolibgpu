...
def ADMM3d_tomobar(
    ...
    if calc_peak_gpu_mem:
        return np.prod(data) * np.float32().itemsize + RecToolsCP.ADMM(
            _data_, _algorithm_, _regularisation_, calc_peak_gpu_mem=calc_peak_gpu_mem
        )
...
def _instantiate_iterative_recon_class(
    ...
    """
    shape = getattr(data, "shape", data)
    if center is None:
        center = shape[2] // 2  # making a crude guess
    if recon_size is None:
        recon_size = shape[2]
    if detector_pad is True:
        detector_pad = __estimate_detectorHoriz_padding(shape[2])
    elif detector_pad is False:
        detector_pad = 0
    RecToolsCP = RecToolsIRCuPy(
        DetectorsDimH=shape[2],  # Horizontal detector dimension
        DetectorsDimH_pad=detector_pad,  # padding for horizontal detector
        DetectorsDimV=shape[1],  # Vertical detector dimension (3D case)
        CenterRotOffset=shape[2] / 2
        - center
        - 0.5,  # Center of Rotation scalar or a vector
        AnglesVec=-angles,  # A vector of projection angles in radians
        ObjSize=recon_size,  # Reconstructed object dimensions (scalar)
