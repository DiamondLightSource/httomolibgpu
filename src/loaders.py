...
def standard_tomo(in_file: Path, data_path: str, image_key_path: str,
    ...
    angles_degrees = get_angles(in_file, comm=comm)
...
def read_through_dim3(file: str, path: str, preview: str=":,:,:",
    ...
    with h5py.File(file, "r", driver="mpio", comm=comm) as in_file:
...
def get_num_chunks(filepath: str, path: str, comm: MPI.Comm) -> int:
    ...
    int
        ADD DESC
    """
    with h5py.File(filepath, "r", driver="mpio", comm=comm) as in_file:
        dataset = in_file[path]
        shape = dataset.shape
...
def get_angles(file: str, path: str="/entry1/tomo_entry/data/rotation_angle",
    ...
    """
    with h5py.File(file, "r", driver="mpio", comm=comm) as file:
        angles = file[path][...]
    return angles
